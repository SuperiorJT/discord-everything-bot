...
impl SqlPollQueries {
    ...
    pub async fn fetch_polls(&self, cache: &InMemoryCache) -> Result<Vec<Poll>, Box<dyn Error>> {
        let conn = self.0.lock().await;
        let polls = conn.prepare("SELECT * from poll")?.query_map([], |row| {
            Ok(Poll {
                id: row.get(0)?,
    ...
    pub async fn delete_poll(&self, id: i64) -> Result<(), Box<dyn Error>> {
        let conn = self.0.lock().await;
        conn.execute(
            "DELETE poll WHERE id = ?",
            params![id])?;
        Ok(())
    }

    pub async fn fetch_poll_options(&self, guild_id: GuildId) -> Result<((EmojiId, String), (EmojiId, String)), Box<dyn Error>> {
        let conn = self.0.lock().await;
    ...
    pub async fn upsert_poll_options(&self, guild_id: GuildId, positive: (EmojiId, String), negative: (EmojiId, String)) -> Result<(), Box<dyn Error>> {
        let conn = self.0.lock().await;
        conn.execute(
            "
            ...
            ",
            params![guild_id.0, positive.0.0, positive.1, negative.0.0, negative.1]
        )?;
        Ok(())
    }
}