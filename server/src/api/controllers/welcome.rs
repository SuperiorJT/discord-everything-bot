...
use crate::{
    api::{models::welcome::WelcomeRequestData, util},
    db::{queries::welcome::WelcomeModuleInsert, Database},
    modules::welcome::{
        WelcomeContent, WelcomeExpanded, WelcomeJoinContent, WelcomeJoinDmContent,
        WelcomeJoinRolesContent, WelcomeLeaveContent,
    },
};

pub async fn fetch_module_for_guild(
    guild_id: u64,
    db: Database,
...
pub async fn update_module_for_guild(
    guild_id: u64,
    data: WelcomeRequestData,
    db: Database,
) -> Result<impl warp::Reply, Infallible> {
    ...
    let mut tx = match db.pool.begin().await {
        ...
        Err(err) => {
            return Ok(util::create_error_response(
                ...
                format!(
                    "Internal database error obtaining transaction connection: {:?}",
                    err
                ),
            ));
        }
    ...
    if let Some(join) = data.join {
        if let Some(mut m_join) = module.join.or(Some(WelcomeJoinContent::default())) {
            ...
            if let Err(err) = db
                .welcome()
                .module_join_upsert_with(module.id, &m_join, &mut tx)
                .await
            {
                return Ok(util::create_error_response(
                    StatusCode::INTERNAL_SERVER_ERROR,
    ...
    if let Some(join_dm) = data.join_dm {
        if let Some(mut m_join_dm) = module.join_dm.or(Some(WelcomeJoinDmContent::default())) {
            ...
            if let Err(err) = db
                .welcome()
                .module_join_dm_upsert_with(module.id, &m_join_dm, &mut tx)
                .await
            {
                return Ok(util::create_error_response(
                    StatusCode::INTERNAL_SERVER_ERROR,
    ...
    if let Some(join_roles) = data.join_roles {
        if let Some(mut m_join_roles) = module
            .join_roles
            .or(Some(WelcomeJoinRolesContent::default()))
        {
            m_join_roles.enabled = join_roles.enabled;

            if let Err(err) = db
                .welcome()
                .module_join_roles_upsert_with(module.id, &m_join_roles, &mut tx)
                .await
            {
                return Ok(util::create_error_response(
                    StatusCode::INTERNAL_SERVER_ERROR,
    ...
    if let Some(leave) = data.leave {
        if let Some(mut m_leave) = module.leave.or(Some(WelcomeLeaveContent::default())) {
            ...
            if let Err(err) = db
                .welcome()
                .module_leave_update_with(module.id, &m_leave, &mut tx)
                .await
            {
                return Ok(util::create_error_response(
                    StatusCode::INTERNAL_SERVER_ERROR,
    ...
    if let Err(err) = tx.commit().await {
        return Ok(util::create_error_response(
            StatusCode::INTERNAL_SERVER_ERROR,
            format!(
                "Internal database error while committing welcome module update: {:?}",
                err
            ),
        ));
    }
...
pub async fn upload_images_for_guild(
    guild_id: u64,
    form: FormData,
) -> Result<impl warp::Reply, Infallible> {
    ...
    for p in parts {
        let name = match p.name() {
            "join.author.image" | "join.image" | "join.thumbnail" | "join.footer.image" => {
                p.name().to_string()
            }
            _ => {
...
pub async fn _fetch_expanded_module(
    db: &Database,
    guild_id: u64,
) -> Result<WelcomeExpanded, warp::reply::Response> {
    match db
        .welcome()
        .module_expanded_fetch_by_guild_id(guild_id)
        .await
    {
        Ok(m) => Ok(m),
        Err(err) => {
            if let sqlx::Error::RowNotFound = err {
                if let Ok(module) = db
                    .welcome()
                    .module_insert(WelcomeModuleInsert {
                        guild_id,
                        enabled: false,
                    })
                    .await
                ...
                return Err(util::create_error_response(
                    StatusCode::INTERNAL_SERVER_ERROR,
                    format!(
                        "Internal database error when fetching expanded welcome module: {:?}",
                        err
                    ),
                ));
            }
